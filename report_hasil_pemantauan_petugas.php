...
$pdf->Cell(0, 0.7, 'LAPORAN HASIL PEMANTAUAN PETUGAS', 0, 1, 'C');
...
$pdf->Cell(4, 0.8, 'Nama Petugas', 1, 0, 'C');
$pdf->Cell(2.7, 0.8, 'Kode Lokasi', 1, 0, 'C');
$pdf->Cell(3.2, 0.8, 'Nama Lokasi', 1, 0, 'C');
$pdf->Cell(1.8, 0.8, 'Debit Air', 1, 0, 'C');
$pdf->Cell(1, 0.8, 'pH', 1, 0, 'C');
$pdf->Cell(1, 0.8, 'BOD', 1, 0, 'C');
$pdf->Cell(1, 0.8, 'COD', 1, 0, 'C');
$pdf->Cell(1, 0.8, 'TSS', 1, 0, 'C');
$pdf->Cell(1, 0.8, 'DO', 1, 1, 'C');
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell(4, 0.6, $row['nama_petugas'], 1, 0, 'C');
    $pdf->Cell(2.7, 0.6, $row['kode_lokasi'], 1, 0, 'C');
    $pdf->Cell(3.2, 0.6, $row['nama_lokasi'], 1, 0, 'C');
    $pdf->Cell(1.8, 0.6, $row['debit_air'], 1, 0, 'C');
    $pdf->Cell(1, 0.6, $row['ph'], 1, 0, 'C');
    $pdf->Cell(1, 0.6, $row['bod'], 1, 0, 'C');
    $pdf->Cell(1, 0.6, $row['cod'], 1, 0, 'C');
    $pdf->Cell(1, 0.6, $row['tss'], 1, 0, 'C');
    $pdf->Cell(1, 0.6, $row['do'], 1, 1, 'C');
}
...
header("Content-Disposition: attachment; filename=Report_Hasil_Pemantauan_Petugas.pdf");

// ============================
// OUTPUT
// ============================
$pdf->Output('D', 'Report_Hasil_Pemantauan_Petugas.pdf');
exit;
