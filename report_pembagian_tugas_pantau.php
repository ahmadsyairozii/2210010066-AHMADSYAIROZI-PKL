...
$pdf->Cell(0, 0.7, 'LAPORAN PEMBAGIAN TUGAS PANTAU', 0, 1, 'C');
...
$pdf->Cell(3, 0.8, 'Nama Petugas', 1, 0, 'C');
$pdf->Cell(2, 0.8, 'Nomor Hp', 1, 0, 'C');
$pdf->Cell(3.4, 0.8, 'Tanggal Pemantauan', 1, 0, 'C');
$pdf->Cell(2, 0.8, 'Kode Lokasi', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'Nama Lokasi', 1, 0, 'C');
$pdf->Cell(4, 0.8, 'Alamat Lokasi', 1, 1, 'C');
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell(3, 0.6, $row['nama_petugas'], 1, 0, 'C');
    $pdf->Cell(2, 0.6, $row['no_hp'], 1, 0, 'C');
    $pdf->Cell(3.4, 0.6, date('d-m-Y', strtotime($row['tanggal_pemantauan'])), 1, 0, 'C');
    $pdf->Cell(2, 0.6, $row['kode_lokasi'], 1, 0, 'C');
    $pdf->Cell(2.5, 0.6, $row['nama_lokasi'], 1, 0, 'C');
    $pdf->Cell(4, 0.6, $row['alamat_lokasi'], 1, 1, 'C');
}
...
header("Content-Disposition: attachment; filename=Report_Pembagian_Tugas_Pantau.pdf");

// ============================
// OUTPUT
// ============================
$pdf->Output('D', 'Report_Pembagian_Tugas_Pantau.pdf');
exit;
