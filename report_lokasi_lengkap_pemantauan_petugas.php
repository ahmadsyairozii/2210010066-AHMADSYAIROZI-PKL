...
$pdf->Cell(0, 0.7, 'LAPORAN LOKASI LENGKAP PEMANTAUAN PETUGAS', 0, 1, 'C');
...
$pdf->Cell(2.5, 0.8, 'Nama Lokasi', 1, 0, 'C');
$pdf->Cell(3.5, 0.8, 'Alamat Lokasi', 1, 0, 'C');
$pdf->Cell(3, 0.8, 'Kabupaten/Kota', 1, 0, 'C');
$pdf->Cell(2.7, 0.8, 'Provinsi', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'Latitude', 1, 0, 'C');
$pdf->Cell(2, 0.8, 'Longitude', 1, 1, 'C');
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell(2.5, 0.6, $row['nama_lokasi'], 1, 0, 'C');
    $pdf->Cell(3.5, 0.6, $row['alamat_lokasi'], 1, 0, 'C');
    $pdf->Cell(3, 0.6, $row['kabupaten_kota'], 1, 0, 'C');
    $pdf->Cell(2.7, 0.6, $row['provinsi'], 1, 0, 'C');
    $pdf->Cell(2.5, 0.6, $row['latitude'], 1, 0, 'C');
    $pdf->Cell(2, 0.6, $row['longitude'], 1, 1, 'C');
}
...
header("Content-Disposition: attachment; filename=Report_Lokasi_Lengkap_Pemantauan_Petugas.pdf");

// ============================
// OUTPUT
// ============================
$pdf->Output('D', 'Report_Lokasi_Lengkap_Pemantauan_Petugas.pdf');
exit;
